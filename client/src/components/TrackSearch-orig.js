...
const TrackSearch = ({ searchTerm }) => {
  ...
  const processedTracks = filteredTracks.map((track) => {
    // console.log('Processing track:', trackDetails[track.id]);
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      sortable: false,
      renderCell: (params) => {
        const segments = params.row.segments || [];
        const beats = params.row.beats || [];
        const sections = params.row.sections || [];
        return <Waveform segments={segments} beats={beats} sections={sections} />;
      },
    },
    {
      field: 'name',
  ...
  ], [selectAllChecked, selectedTrackIds, trackDetails]);
